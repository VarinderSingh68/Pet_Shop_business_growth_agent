...
A pet shop storefront, admin panel, and Growth Agent marketing engine — hand-rolled PHP MVC (no framework), SQLite, server-rendered views with a locally-built Tailwind CSS bundle and Alpine.js. Built to deploy anywhere PHP runs, including plain shared hosting — the whole database is a single file, no separate database server to install or configure.
...
- Composer
- Node.js 20+ (only for building the Tailwind CSS bundle — not needed at runtime)

## Setup

```bash
composer install
npm install && npm run build   # compiles resources/css/tailwind.css -> public/assets/build/tailwind.css
cp .env.example .env
```

Re-run `npm run build` (or `npm run dev` to watch for changes) any time you add new utility classes to a view — the compiled CSS is not committed, so a fresh clone has none until you build it once.
...
```bash
composer test       # PHPUnit — runs against an isolated, migrated SQLite DB in the system temp dir
composer analyse     # PHPStan (level 5); phpstan-baseline.neon tracks pre-existing findings not yet cleaned up
composer lint        # PHP-CS-Fixer, dry-run
composer lint:fix    # PHP-CS-Fixer, applies fixes
```

`.github/workflows/ci.yml` runs all of the above (plus the asset build) on every push and pull request. The suite is intentionally focused on core logic (`Validator`, `Database`, `User`, `CartService` pricing math) rather than full controller/HTTP coverage — expanding it is the natural next step as the app grows.

## Project structure

```
public/          Single entry point (index.php), built assets (assets/build/, generated — not committed), uploads-facing dir (empty; real uploads live in storage/)
app/
...
resources/css/   Tailwind source (tailwind.css, tailwind.config.js at repo root) — build with `npm run build`
tests/           PHPUnit — Unit/ (pure logic) and Feature/ (isolated SQLite DB)
storage/         database.sqlite (the whole database, not committed — generated by migrate.php), logs/, cache/, backups/, uploads/ (outside the web root — served via a controller, not committed)
