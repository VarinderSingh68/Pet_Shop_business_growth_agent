...
<section class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10"
  ...
  <div class="mt-6 grid lg:grid-cols-2 gap-12">
    ...
    <div>
      ...
      <?php else: ?>
        <div class="border-2 border-ink aspect-square overflow-hidden cursor-zoom-in" @click="zoom = !zoom">
        ...
        <?php if (count($images) > 1): ?>
          <div class="mt-3 grid grid-cols-5 gap-2">
            <?php foreach (array_slice($images, 1, 5) as $img): ?>
              <div class="border-2 border-ink aspect-square overflow-hidden">
    ...
    <div>
        ...
        <div class="flex flex-wrap gap-2">
          <template x-for="v in variants" :key="v.id">
            <button type="button" @click="selected = v.id"
                    :class="selected === v.id ? 'border-leash bg-leash text-paper' : 'border-ink hover:border-leash'"
                    :disabled="v.stock <= 0"
                    class="border-2 px-4 py-2 text-sm font-medium disabled:opacity-40 disabled:cursor-not-allowed"
      ...
      <form method="POST" action="/cart/add" class="mt-8 flex gap-3">
        ...
        <input type="number" name="qty" value="1" min="1" class="w-20 border-2 border-ink px-3 py-3 text-center">
      ...
      <?php if ($product['feeding_grams_per_day']): ?>
        <div class="mt-6 border-2 border-fern p-4 bg-fern/5">
          <p class="flex items-center gap-2 text-sm font-semibold text-fern"><span class="icon-chip icon-chip-fern !w-7 !h-7"><?= icon('refresh', 'h-3.5 w-3.5') ?></span> Subscribe &amp; never run out</p>
          ...
          <?php if (auth()->check()): ?>
            <form method="POST" action="/account/subscriptions" class="mt-3 flex flex-wrap items-end gap-3">
              ...
              <div>
                ...
                <select id="sub_interval" name="interval_days" class="border-2 border-ink px-2 py-1.5 text-sm bg-paper">
              ...
              <button type="submit" class="btn btn-sm !border-fern !text-fern !shadow-[2px_2px_0_var(--fern)] bg-transparent hover:!bg-fern hover:!text-paper">Subscribe</button>
            </form>
          <?php else: ?>
            <a href="/account/login" class="mt-3 inline-block text-sm font-semibold text-fern hover:underline">Sign in to subscribe</a>
          <?php endif; ?>
        </div>
      <?php endif; ?>

      <?php if ($product['description']): ?>
        <div class="mt-10 border-t-2 border-mist pt-6">
  ...
  <!-- Reviews -->
  <div class="mt-16 border-t-2 border-ink pt-10">
      ...
      <details class="mt-8">
        ...
        <form method="POST" action="/reviews" class="mt-4 max-w-lg space-y-4 card-tag p-5">
          ...
          <div>
            ...
            <select id="review_rating" name="rating" required class="border-2 border-ink px-3 py-2">
          ...
          <div>
            ...
            <input id="review_title" type="text" name="title" class="w-full border-2 border-ink px-3 py-2">
          </div>
          <div>
            ...
            <textarea id="review_body" name="body" rows="3" class="w-full border-2 border-ink px-3 py-2"></textarea>
          </div>
          <button type="submit" class="btn btn-primary">Submit review</button>
        </form>
      </details>
    <?php else: ?>
      <p class="mt-6 text-sm text-ink/60"><a href="/account/login" class="text-leash hover:underline">Sign in</a> to write a review.</p>
    <?php endif; ?>
  </div>
      ...
      <div class="mt-6 grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <?php foreach ($related as $rp): ?>
          <a href="/shop/<?= e($rp['slug']) ?>" class="card-tag card-tag--pop group">
            ...
            <div class="p-4">
              <h3 class="font-semibold text-sm group-hover:text-leash"><?= e($rp['name']) ?></h3>
